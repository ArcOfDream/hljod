...
TabBarFlags_FittingPolicyMask_ :: TabBarFlags {
	.FittingPolicyMixed,
	.FittingPolicyShrink,
	.FittingPolicyScroll,
}
...
MultiSelectFlags_SelectOnMask_ :: MultiSelectFlags {
	.SelectOnAuto,
	.SelectOnClickAlways,
...
PlatformIO :: struct {
	...
	Platform_CreateVkSurface:           proc "c" (
		...
		out_vk_surface: ^u64,
	) -> i32, // (Optional) For a Vulkan Renderer to call into Platform code (since the surface creation needs to tie them both).
...
foreign lib {
	...
	GetDrawData :: proc() -> ^DrawData --- // valid after Render() and until the next call to NewFrame(). Call ImGui_ImplXXXX_RenderDrawData() function in your Renderer Backend to render.
	...
	PushFontFloat :: proc(font: ^Font, font_size_base_unscaled: f32) --- // Use NULL as a shortcut to keep current font. Use 0.0f to keep current size.
	@(link_name = "ImGui_PopFont")
	PopFont :: proc() ---
	@(link_name = "ImGui_GetFont")
	GetFont :: proc() -> ^Font --- // get current font
	@(link_name = "ImGui_GetFontSize")
	GetFontSize :: proc() -> f32 --- // get current scaled font size (= height in pixels). AFTER global scale factors applied. *IMPORTANT* DO NOT PASS THIS VALUE TO PushFont()! Use ImGui::GetStyle().FontSizeBase to get value before global scale factors.
	@(link_name = "ImGui_GetFontBaked")
	GetFontBaked :: proc() -> ^FontBaked --- // get current font bound at current size // == GetFont()->GetFontBaked(GetFontSize())
	...
	PushIDInt :: proc(int_id: i32) --- // push integer into the ID stack (will hash integer).
	...
	TextColoredUnformatted :: proc(col: Vec4, text: cstring) --- // shortcut for PushStyleColor(ImGuiCol_Text, col); Text(fmt, ...); PopStyleColor();
	...
	TextDisabledUnformatted :: proc(text: cstring) --- // shortcut for PushStyleColor(ImGuiCol_Text, style.Colors[ImGuiCol_TextDisabled]); Text(fmt, ...); PopStyleColor();
	...
	TextWrappedUnformatted :: proc(text: cstring) --- // shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note that this won't work on an auto-resizing window if there's no other widgets to extend the window width, yoy may need to set a size using SetNextWindowSize().
	...
	LabelTextUnformatted :: proc(label: cstring, text: cstring) --- // display text+label aligned the same way as value+label widgets
	...
	BulletTextUnformatted :: proc(text: cstring) --- // shortcut for Bullet()+Text()
	...
	RadioButtonIntPtr :: proc(label: cstring, v: ^i32, v_button: i32) -> bool --- // shortcut to handle the above pattern when value is an integer
	@(link_name = "ImGui_ProgressBar")
	ProgressBar :: proc(fraction: f32, size_arg: Vec2 = {-math.F32_MIN, 0}, overlay: cstring = nil) ---
	...
	TextLinkOpenURL :: proc(label: cstring, url: cstring = nil) -> bool --- // hyperlink text button, automatically open file/url when clicked
	// Widgets: Images
	...
	Image :: proc(tex_ref: TextureRef, image_size: Vec2, uv0: Vec2 = {0, 0}, uv1: Vec2 = {1, 1}) ---
	@(link_name = "ImGui_ImageWithBg")
	ImageWithBg :: proc(tex_ref: TextureRef, image_size: Vec2, uv0: Vec2 = {0, 0}, uv1: Vec2 = {1, 1}, bg_col: Vec4 = {0, 0, 0, 0}, tint_col: Vec4 = {1, 1, 1, 1}) ---
	@(link_name = "ImGui_ImageButton")
	ImageButton :: proc(str_id: cstring, tex_ref: TextureRef, image_size: Vec2, uv0: Vec2 = {0, 0}, uv1: Vec2 = {1, 1}, bg_col: Vec4 = {0, 0, 0, 0}, tint_col: Vec4 = {1, 1, 1, 1}) -> bool ---
	...
	Combo :: proc(label: cstring, current_item: ^i32, items_separated_by_zeros: cstring, popup_max_height_in_items: i32 = -1) -> bool --- // Separate items with \0 within a string, end item-list with \0\0. e.g. "One\0Two\0Three\0"
	...
	DragInt :: proc(label: cstring, v: ^i32, v_speed: f32 = 1.0, v_min: i32 = 0, v_max: i32 = 0, format: cstring = "%d", flags: SliderFlags = {}) -> bool --- // If v_min >= v_max we have no bound
	...
	InputTextMultiline :: proc(label: cstring, buf: cstring, buf_size: uint, size: Vec2 = {0, 0}, flags: InputTextFlags = {}, callback: InputTextCallback = nil, user_data: rawptr = nil) -> bool ---
	...
	TreeNodeStr :: proc(str_id: cstring, fmt: cstring, #c_vararg args: ..any) -> bool --- // helper variation to easily decorrelate the id from the displayed string. Read the FAQ about why and how to use ID. to align arbitrary text at the same level as a TreeNode() you can use Bullet().
	@(link_name = "ImGui_TreeNodeStrUnformatted")
	TreeNodeStrUnformatted :: proc(str_id: cstring, text: cstring) -> bool --- // helper variation to easily decorrelate the id from the displayed string. Read the FAQ about why and how to use ID. to align arbitrary text at the same level as a TreeNode() you can use Bullet().
	...
	TreeNodePtrUnformatted :: proc(ptr_id: rawptr, text: cstring) -> bool --- // "
	...
	TreeNodeExStrUnformatted :: proc(str_id: cstring, flags: TreeNodeFlags, text: cstring) -> bool ---
	...
	TreeNodeExPtrUnformatted :: proc(ptr_id: rawptr, flags: TreeNodeFlags, text: cstring) -> bool ---
	...
	TreeNodeGetOpen :: proc(storage_id: ID) -> bool --- // retrieve tree node open/close state.
	// Widgets: Selectables
	...
	PlotLines :: proc(label: cstring, values: ^f32, values_count: i32, values_offset: i32 = 0, overlay_text: cstring = nil, scale_min: f32 = math.F32_MAX, scale_max: f32 = math.F32_MAX, graph_size: Vec2 = {0, 0}, stride: i32 = size_of(f32)) ---
	@(link_name = "ImGui_PlotLinesCallback")
	PlotLinesCallback :: proc(label: cstring, values_getter: proc "c" (data: rawptr, idx: i32) -> f32, data: rawptr, values_count: i32, values_offset: i32 = 0, overlay_text: cstring = nil, scale_min: f32 = math.F32_MAX, scale_max: f32 = math.F32_MAX, graph_size: Vec2 = {0, 0}) ---
	@(link_name = "ImGui_PlotHistogram")
	PlotHistogram :: proc(label: cstring, values: ^f32, values_count: i32, values_offset: i32 = 0, overlay_text: cstring = nil, scale_min: f32 = math.F32_MAX, scale_max: f32 = math.F32_MAX, graph_size: Vec2 = {0, 0}, stride: i32 = size_of(f32)) ---
	@(link_name = "ImGui_PlotHistogramCallback")
	PlotHistogramCallback :: proc(label: cstring, values_getter: proc "c" (data: rawptr, idx: i32) -> f32, data: rawptr, values_count: i32, values_offset: i32 = 0, overlay_text: cstring = nil, scale_min: f32 = math.F32_MAX, scale_max: f32 = math.F32_MAX, graph_size: Vec2 = {0, 0}) ---
	// Widgets: Menus
	...
	SetTooltipUnformatted :: proc(text: cstring) --- // set a text-only tooltip. Often used after a ImGui::IsItemHovered() check. Override any previous call to SetTooltip().
	...
	SetItemTooltipUnformatted :: proc(text: cstring) --- // set a text-only tooltip if preceding item was hovered. override any previous call to SetTooltip().
	// Popups, Modals
	...
	OpenPopupOnItemClick :: proc(str_id: cstring = nil, popup_flags: PopupFlags = {}) --- // helper to open popup when clicked on last item. Default to ImGuiPopupFlags_MouseButtonRight == 1. (note: actually triggers on the mouse _released_ event to be consistent with popup behaviors)
	...
	BeginPopupContextItem :: proc(str_id: cstring = nil, popup_flags: PopupFlags = {}) -> bool --- // open+begin popup when clicked on last item. Use str_id==NULL to associate the popup to previous item. If you want to use that on a non-interactive item such as Text() you need to pass in an explicit ID here. read comments in .cpp!
	@(link_name = "ImGui_BeginPopupContextWindow")
	BeginPopupContextWindow :: proc(str_id: cstring = nil, popup_flags: PopupFlags = {}) -> bool --- // open+begin popup when clicked on current window.
	@(link_name = "ImGui_BeginPopupContextVoid")
	BeginPopupContextVoid :: proc(str_id: cstring = nil, popup_flags: PopupFlags = {}) -> bool --- // open+begin popup when clicked in void (where there are no windows).
	...
	BeginTable :: proc(str_id: cstring, columns: i32, flags: TableFlags = {}, outer_size: Vec2 = {0.0, 0.0}, inner_width: f32 = 0.0) -> bool ---
	@(link_name = "ImGui_EndTable")
	EndTable :: proc() --- // only call EndTable() if BeginTable() returns true!
	@(link_name = "ImGui_TableNextRow")
	TableNextRow :: proc(row_flags: TableRowFlags = {}, min_row_height: f32 = 0.0) --- // append into the first cell of a new row. 'min_row_height' include the minimum top and bottom padding aka CellPadding.y * 2.0f.
	...
	TableSetColumnIndex :: proc(column_n: i32) -> bool --- // append into the specified column. Return true when column is visible.
	...
	TableSetupScrollFreeze :: proc(cols: i32, rows: i32) --- // lock columns/rows so they stay visible when scrolled.
	...
	TableGetColumnCount :: proc() -> i32 --- // return number of columns (value passed to BeginTable)
	@(link_name = "ImGui_TableGetColumnIndex")
	TableGetColumnIndex :: proc() -> i32 --- // return current column index.
	@(link_name = "ImGui_TableGetRowIndex")
	TableGetRowIndex :: proc() -> i32 --- // return current row index (header rows are accounted for)
	@(link_name = "ImGui_TableGetColumnName")
	TableGetColumnName :: proc(column_n: i32 = -1) -> cstring --- // return "" if column didn't have a name declared by TableSetupColumn(). Pass -1 to use current column.
	@(link_name = "ImGui_TableGetColumnFlags")
	TableGetColumnFlags :: proc(column_n: i32 = -1) -> TableColumnFlags --- // return column flags so you can query their Enabled/Visible/Sorted/Hovered status flags. Pass -1 to use current column.
	@(link_name = "ImGui_TableSetColumnEnabled")
	TableSetColumnEnabled :: proc(column_n: i32, v: bool) --- // change user accessible enabled/disabled state of a column. Set to false to hide the column. User can use the context menu to change this themselves (right-click in headers, or right-click in columns body with ImGuiTableFlags_ContextMenuInBody)
	@(link_name = "ImGui_TableGetHoveredColumn")
	TableGetHoveredColumn :: proc() -> i32 --- // return hovered column. return -1 when table is not hovered. return columns_count if the unused space at the right of visible columns is hovered. Can also use (TableGetColumnFlags() & ImGuiTableColumnFlags_IsHovered) instead.
	@(link_name = "ImGui_TableSetBgColor")
	TableSetBgColor :: proc(target: TableBgTarget, color: u32, column_n: i32 = -1) --- // change the color of a cell, row, or column. See ImGuiTableBgTarget_ flags for details.
	...
	GetColumnIndex :: proc() -> i32 --- // get current column index
	@(link_name = "ImGui_GetColumnWidth")
	GetColumnWidth :: proc(column_index: i32 = -1) -> f32 --- // get column width (in pixels). pass -1 to use current column
	@(link_name = "ImGui_SetColumnWidth")
	SetColumnWidth :: proc(column_index: i32, width: f32) --- // set column width (in pixels). pass -1 to use current column
	@(link_name = "ImGui_GetColumnOffset")
	GetColumnOffset :: proc(column_index: i32 = -1) -> f32 --- // get position of column line (in pixels, from the left side of the contents region). pass -1 to use current column, otherwise 0..GetColumnsCount() inclusive. column 0 is typically 0.0f
	@(link_name = "ImGui_SetColumnOffset")
	SetColumnOffset :: proc(column_index: i32, offset_x: f32) --- // set position of column line (in pixels, from the left side of the contents region). pass -1 to use current column
	...
	GetWindowDockID :: proc() -> ID --- // get dock id of current window, or 0 if not associated to any docking node.
	...
	LogToTTY :: proc(auto_open_depth: i32 = -1) --- // start logging to tty (stdout)
	@(link_name = "ImGui_LogToFile")
	LogToFile :: proc(auto_open_depth: i32 = -1, filename: cstring = nil) --- // start logging to file
	@(link_name = "ImGui_LogToClipboard")
	LogToClipboard :: proc(auto_open_depth: i32 = -1) --- // start logging to OS clipboard
	...
	LogTextUnformatted :: proc(text: cstring) --- // pass text data straight to log (without being displayed)
	// Drag and Drop
	...
	SetDragDropPayload :: proc(type: cstring, data: rawptr, sz: uint, cond: Cond = {}) -> bool --- // type is a user defined string of maximum 32 characters. Strings starting with '_' are reserved for dear imgui internal types. Data is copied and held by imgui. Return true when payload has been accepted.
	...
	SetKeyboardFocusHere :: proc(offset: i32 = 0) --- // focus keyboard on the next widget. Use positive 'offset' to access sub components of a multiple component widget. Use -1 to access previous widget.
	...
	SetNextItemAllowOverlap :: proc() --- // allow next item to be overlapped by a subsequent item. Typically useful with InvisibleButton(), Selectable(), TreeNode() covering an area where subsequent items may need to be added. Note that both Selectable() and TreeNode() have dedicated flags doing this.
	...
	GetItemFlags :: proc() -> ItemFlags --- // get generic flags of last item
	// Viewports
	// - Currently represents the Platform Window created by the application which is hosting our Dear ImGui windows.
	...
	GetFrameCount :: proc() -> i32 --- // get global imgui frame count. incremented by 1 every frame.
	...
	IsKeyPressed :: proc(key: Key, repeat: bool = true) -> bool --- // was key pressed (went from !Down to Down)? Repeat rate uses io.KeyRepeatDelay / KeyRepeatRate.
	...
	GetKeyPressedAmount :: proc(key: Key, repeat_delay: f32, rate: f32) -> i32 --- // uses provided repeat rate/delay. return a count, most often 0 or 1 but might be >1 if RepeatRate is small enough that DeltaTime > RepeatRate
	...
	SetItemKeyOwner :: proc(key: Key) -> bool --- // Set key owner to last item ID if it is hovered or active. Return true when ownership has been set. Roughly equivalent to 'if (TestKeyOwner(key, GetItemID()) && (IsItemHovered() || IsItemActive())) { SetKeyOwner(key, GetItemID());'.
	...
	GetMouseClickedCount :: proc(button: MouseButton) -> i32 --- // return the number of successive mouse-clicks at the time where a click happen (otherwise 0).
	...
	SetNextFrameWantCaptureMouse :: proc(want_capture_mouse: bool) --- // Override io.WantCaptureMouse flag next frame (said flag is left for your application to handle, typical when true it instructs your app to ignore inputs). This is equivalent to setting "io.WantCaptureMouse = want_capture_mouse;" after the next NewFrame() call.
	// Clipboard Utilities
	...
	LoadIniSettingsFromMemory :: proc(ini_data: cstring, ini_size: uint = 0) --- // call after CreateContext() and before the first call to NewFrame() to provide .ini data from your own data source.
	...
	SaveIniSettingsToMemory :: proc(out_ini_size: ^uint = nil) -> cstring --- // return a zero-terminated string with the .ini data which you can save by your own mean. call when io.WantSaveIniSettings is set, then save data by your own mean and clear io.WantSaveIniSettings.
	// Debug Utilities
	...
	DebugCheckVersionAndDataLayout :: proc(version_str: cstring, sz_io: uint, sz_style: uint, sz_vec2: uint, sz_vec4: uint, sz_drawvert: uint, sz_drawidx: uint) -> bool --- // This is called by IMGUI_CHECKVERSION() macro.
	...
	DebugLogUnformatted :: proc(text: cstring) --- // Call via IMGUI_DEBUG_LOG() for maximum stripping in caller code!
	// Memory Allocators
	...
	FindViewportByID :: proc(viewport_id: ID) -> ^Viewport --- // this is a helper for backends.
	...
	PlatformIO_SetPlatform_GetWindowWorkAreaInsets :: proc(getWindowWorkAreaInsetsFunc: proc "c" (vp: ^Viewport, result: ^Vec4)) --- // Set ImGuiPlatformIO::Platform_GetWindowWorkAreaInsets in a C-compatible mannner
	@(link_name = "ImGuiPlatformIO_SetPlatform_GetWindowFramebufferScale")
	PlatformIO_SetPlatform_GetWindowFramebufferScale :: proc(getWindowFramebufferScaleFunc: proc "c" (vp: ^Viewport, result: ^Vec2)) --- // Set ImGuiPlatformIO::Platform_GetWindowFramebufferScale in a C-compatible mannner
	@(link_name = "ImGuiPlatformIO_SetPlatform_GetWindowPos")
	PlatformIO_SetPlatform_GetWindowPos :: proc(getWindowPosFunc: proc "c" (vp: ^Viewport, result: ^Vec2)) --- // Set ImGuiPlatformIO::Platform_GetWindowPos in a C-compatible mannner
	@(link_name = "ImGuiPlatformIO_SetPlatform_GetWindowSize")
	PlatformIO_SetPlatform_GetWindowSize :: proc(getWindowSizeFunc: proc "c" (vp: ^Viewport, result: ^Vec2)) --- // Set ImGuiPlatformIO::Platform_GetWindowSize in a C-compatible mannner
	@(link_name = "ImGuiStyle_ScaleAllSizes")
	Style_ScaleAllSizes :: proc(self: ^Style, scale_factor: f32) --- // Scale all spacing/padding/thickness values. Do not scale fonts. See comments in definition. Consider not calling this if your initial scale factor if <1.0.
	// Input Functions
	...
	IO_AddMouseButtonEvent :: proc(self: ^IO, button: i32, down: bool) --- // Queue a mouse button change
	...
	IO_AddInputCharacter :: proc(self: ^IO, _c: u32) --- // Queue a new character input
	...
	IO_SetKeyEventNativeData :: proc(self: ^IO, key: Key, native_keycode: i32, native_scancode: i32, native_legacy_index: i32 = -1) --- // [Optional] Specify index for legacy <1.87 IsKeyXXX() functions with native indices + specify native keycode, scancode.
	...
	TextFilter_ImGuiTextRange_split :: proc(self: ^TextFilter_ImGuiTextRange, separator: u8, out: ^Vector_TextRange) ---
	...
	TextBuffer_resize :: proc(self: ^TextBuffer, size: i32) --- // Similar to resize(0) on ImVector: empty string but don't free buffer.
	...
	ListClipper_IncludeItemsByIndex :: proc(self: ^ListClipper, item_begin: i32, item_end: i32) --- // item_end is exclusive e.g. use (42, 42+1) to make item 42 never clipped.
	...
	SelectionBasicStorage_GetStorageIdFromIndex :: proc(self: ^SelectionBasicStorage, idx: i32) -> ID --- // Convert index to item id based on provided adapter.
	...
	DrawList_AddRect :: proc(self: ^DrawList, p_min: Vec2, p_max: Vec2, col: u32, rounding: f32 = 0.0, thickness: f32 = 1.0, flags: DrawFlags = {}) --- // a: upper-left, b: lower-right (== upper-left + size)
	...
	DrawList_AddImage :: proc(self: ^DrawList, tex_ref: TextureRef, p_min: Vec2, p_max: Vec2, uv_min: Vec2 = {0, 0}, uv_max: Vec2 = {1, 1}, col: u32 = 0xff_ff_ff_ff) ---
	@(link_name = "ImDrawList_AddImageQuad")
	DrawList_AddImageQuad :: proc(self: ^DrawList, tex_ref: TextureRef, p1: Vec2, p2: Vec2, p3: Vec2, p4: Vec2, uv1: Vec2 = {0, 0}, uv2: Vec2 = {1, 0}, uv3: Vec2 = {1, 1}, uv4: Vec2 = {0, 1}, col: u32 = 0xff_ff_ff_ff) ---
	@(link_name = "ImDrawList_AddImageRounded")
	DrawList_AddImageRounded :: proc(self: ^DrawList, tex_ref: TextureRef, p_min: Vec2, p_max: Vec2, uv_min: Vec2, uv_max: Vec2, col: u32, rounding: f32, flags: DrawFlags = {}) ---
	...
	DrawList_PathEllipticalArcTo :: proc(self: ^DrawList, center: Vec2, radius: Vec2, rot: f32, a_min: f32, a_max: f32, num_segments: i32 = 0) --- // Ellipse
	...
	DrawList_AddCallback :: proc(self: ^DrawList, callback: DrawCallback, userdata: rawptr = nil, userdata_size: uint = 0) ---
	...
	FontGlyphRangesBuilder_GetBit :: proc(self: ^FontGlyphRangesBuilder, n: uint) -> bool --- // Get bit n in the array
	@(link_name = "ImFontGlyphRangesBuilder_SetBit")
	FontGlyphRangesBuilder_SetBit :: proc(self: ^FontGlyphRangesBuilder, n: uint) --- // Set bit n in the array
	...
	FontAtlas_AddFontDefault :: proc(self: ^FontAtlas, font_cfg: ^FontConfig = nil) -> ^Font --- // Selects between AddFontDefaultVector() and AddFontDefaultBitmap().
	@(link_name = "ImFontAtlas_AddFontDefaultVector")
	FontAtlas_AddFontDefaultVector :: proc(self: ^FontAtlas, font_cfg: ^FontConfig = nil) -> ^Font --- // Embedded scalable font. Recommended at any higher size.
	@(link_name = "ImFontAtlas_AddFontDefaultBitmap")
	FontAtlas_AddFontDefaultBitmap :: proc(self: ^FontAtlas, font_cfg: ^FontConfig = nil) -> ^Font --- // Embedded classic pixel-clean font. Recommended at Size 13px with no scaling.
	...
	FontAtlas_AddFontFromMemoryCompressedTTF :: proc(self: ^FontAtlas, compressed_font_data: rawptr, compressed_font_data_size: i32, size_pixels: f32 = 0.0, font_cfg: ^FontConfig = nil, glyph_ranges: ^Wchar = nil) -> ^Font --- // 'compressed_font_data' still owned by caller. Compress with binary_to_compressed_c.cpp.
	@(link_name = "ImFontAtlas_AddFontFromMemoryCompressedBase85TTF")
	FontAtlas_AddFontFromMemoryCompressedBase85TTF :: proc(self: ^FontAtlas, compressed_font_data_base85: cstring, size_pixels: f32 = 0.0, font_cfg: ^FontConfig = nil, glyph_ranges: ^Wchar = nil) -> ^Font --- // 'compressed_font_data_base85' still owned by caller. Compress with binary_to_compressed_c.cpp with -base85 parameter.
	@(link_name = "ImFontAtlas_RemoveFont")
	FontAtlas_RemoveFont :: proc(self: ^FontAtlas, font: ^Font) ---
	@(link_name = "ImFontAtlas_Clear")
	FontAtlas_Clear :: proc(self: ^FontAtlas) --- // Clear everything (fonts + textures). Don't call mid-frame!
	@(link_name = "ImFontAtlas_ClearFonts")
	FontAtlas_ClearFonts :: proc(self: ^FontAtlas) --- // Clear input+output font data/glyphs. You can call this mid-frame if you load new fonts afterwards!
	...
	FontAtlas_AddCustomRect :: proc(self: ^FontAtlas, width: i32, height: i32, out_r: ^FontAtlasRect = nil) -> FontAtlasRectId --- // Register a rectangle. Return -1 (ImFontAtlasRectId_Invalid) on error.
	...
	Font_GetFontBaked :: proc(self: ^Font, font_size: f32, density: f32 = -1.0) -> ^FontBaked --- // Get or create baked data for given size
	...
	Font_RenderChar :: proc(self: ^Font, draw_list: ^DrawList, size: f32, pos: Vec2, col: u32, _c: Wchar, cpu_fine_clip: ^Vec4 = nil) ---
	...
	PlatformIO_ClearPlatformHandlers :: proc(self: ^PlatformIO) --- // Clear all Platform_XXX fields. Typically called on Platform Backend shutdown.
	@(link_name = "ImGuiPlatformIO_ClearRendererHandlers")
	PlatformIO_ClearRendererHandlers :: proc(self: ^PlatformIO) --- // Clear all Renderer_XXX fields. Typically called on Renderer Backend shutdown.
}
